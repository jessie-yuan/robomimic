...
def compose_actions(action1, action2):
    action = np.zeros(7)
    # action[:3] = (action1[:3] + action2[:3])
    # action[3:6] = (action1[3:6] + action2[3:6]) / 2
    action[:6] = (action1[:6] + action2[:6])
    action[6] = max(action1[6], action2[6])
    return action

def extract_trajectory_with_downsampling(
    env, 
    initial_state, 
    states, 
    actions,
    ...
    # assert initial_state['states'][12] == 0.89
    # initial_state['states'][12] = 0.83
    ...
    # downsampled_actions = [compose_actions(actions[i], actions[i+1]) for i in range(0, len(actions)-1, 2)]
    # if len(actions) % 2 == 1:
    ...
    # for t in range(2, traj_len + 2, 2):

    idxs_mask = np.append(idxs_mask, True)

    for t, keep in enumerate(idxs_mask):
        ...
        # if t < 10:
        #     assert .8 < states[t][12] and states[t][12] < .89
        #     states[t][12] = 0.83 # correct for square nut drop

        # if t < 100:
        #     print("action {}".format(t), actions[t - 1])
...
def dataset_states_to_obs(args):
    ...
    abs_env = EnvUtils.create_env_for_data_processing(
    ...
    # print(abs_env.env.robots[0].keys())
    # assert not abs_env.env.robots[0].controller.use_delta

    print("==== Using environment with the following metadata ====")
    print(json.dumps(abs_env.serialize(), indent=4))
    print("")
    ...
    for ind in tqdm(range(len(demos))):
        # if ind in [2, 14, 18, 19, 22, 24, 28, 29, 31, 33, 36]: #startback_endleft
        # if ind in [3, 12, 19, 36, 37]: #startback_endright
        # if ind in [3, 8, 10, 14, 20, 22, 24, 25, 36]: #startback_endleft_pt2
        if ind in [6, 11, 13, 15, 26, 36] or ind > 29: #startback_endright_pt2
            # skip these demos, they are not good
            continue
        ...
        traj, camera_info = extract_trajectory_with_downsampling(
            env=env, 
    ...
    data_grp.attrs["env_args"] = json.dumps(abs_env.serialize(), indent=4) # environment info
