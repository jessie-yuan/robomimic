...
def extract_trajectory_with_downsampling(
    ...
    traj = dict(
        ...
        states=np.array(states)[::2],
        initial_state_dict=initial_state,
    )
    traj_len = states.shape[0]
    # iteration variable @t is over "next obs" indices

    #print first 5 actions and states
    print("First 5 actions: {}".format(actions[:5]))
    print("First 5 states: {}".format(states[:5]))

    for t in range(1, traj_len + 1, 2):

        # if t == 2:
        #     env.reset_to({"states" : states[t - 1]})
        #     new_obs, _, _, _ = env.step(actions[t - 1])
        #     print("eef_from_stepping", new_obs['robot0_eef_pos'])
            
        #     new_obs = env.reset_to({"states" : states[t]})
        #     print("eef_from_reset", new_obs['robot0_eef_pos'])

        # get next observation
        if t == traj_len or t == traj_len - 1:
            # play final action to get next observation for last timestep
            next_obs, _, _, _ = env.step(actions[t - 2])
        else:
